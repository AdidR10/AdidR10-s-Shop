...
exports.uploadProfilePhoto = (request, response) => {
	...
	busboy.on('finish', () => {
		admin
			...
			.then(() => {
				const imageUrl = process.env.FUNCTIONS_EMULATOR 
					? `http://localhost:9199/v0/b/${config.storageBucket}/o/${imageFileName}?alt=media`
					: `https://firebasestorage.googleapis.com/v0/b/${config.storageBucket}/o/${imageFileName}?alt=media`;
				return database.doc(`/users/${request.user.email}`).update({
					imageUrl
				});
			})
			.then(() => {
...
exports.updateUserDetails = (request, response) => {
	let document = database.collection('users').doc(`${request.user.email}`);
	document
		.update(request.body)
